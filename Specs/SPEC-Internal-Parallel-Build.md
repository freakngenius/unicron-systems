...
DISCOVERY FIRST, do not assume paths. Locate the app under /Pathfinder, the ui_plan renderer and the /[slug] route, the org-loading code that reads architecture, the per-org Inngest dispatch, and Zedcor's existing components (the quality reference). Report real paths in the PR with verbatim evidence per change. No hypothesis-driven edits.
...
- Types: ModuleDefinition, Slot, Dependency, OrgModuleEntry.
...
- catalog/registry.ts exporting MODULE_REGISTRY with eleven modules. Component refs are lazy stubs that render the floor for now (later streams replace them). Dependency declarations are REAL (id, slot, agent, deps as kind/ref/gate, fallback):
...
- Renderer slot resolution: enabled and all gates met -> module active; enabled and a soft dep unmet -> module inactive fallback; enabled and a hard dep unmet, or no module claims the slot -> ui_plan floor. Never blank; log a misconfiguration and fall back, do not crash.
...
PART 5, design primitives calibrated to the Zedcor bar. Read Zedcor's components as reference and export a shared primitive set: design tokens, a card shell, a score badge, a one-line "why" element, a designed empty-state, and a section header. This is the visual floor the surface streams build on.
