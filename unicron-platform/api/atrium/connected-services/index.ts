...
//
// Uses ns_list_connected_services / ns_create_connected_service RPCs
// (public schema, SECURITY DEFINER) — nervous_system not in PostgREST whitelist.
// Sprint 5 Stream F.
...
async function auditLog(action: string, payload: Record<string, unknown>): Promise<void> {
  try {
    ...
    await sb.rpc('ns_append_ledger_signal', {
      p_source_type: 'audit',
      p_source_id: 'atrium/connected-services',
      p_summary: action,
      p_insights: payload,
    });
  } catch { /* non-fatal */ }
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  if (req.method === 'GET') {
    try {
      const sb = getServiceClient();
      const { data, error } = await sb.rpc('ns_list_connected_services');
  ...
  if (req.method === 'POST') {
    ...
    try {
      const sb = getServiceClient();
      const { data, error } = await sb.rpc('ns_create_connected_service', {
        p_name: body.name,
        p_category: body.category ?? null,
        p_status: body.status ?? 'unknown',
      ...
      await auditLog('insert', { service_id: service?.id, name: body.name });
