...
async function step2(ctx: SmokeContext): Promise<Omit<Result, 'step' | 'name' | 'latency_ms'>> {
  ...
  const { data, error } = await ctx.supabaseService
    ...
    .insert({
      ...
      customer_org_id: ctx.testorg.slug, // legacy text key; NOT NULL alongside uuid id
      status: 'setting_up',
  ...
  return {
    status: 'pass',
    details: { id: data.id, slug: data.slug, status: data.status },
  };
}
...
async function step3(ctx: SmokeContext): Promise<Omit<Result, 'step' | 'name' | 'latency_ms'>> {
  // The auto-dispatch of org.created on insert into pathfinder.organizations is
  // wired in Phase 2E. Probe: did agent_runs / agent_log get any rows for this
  // org_id within ~6s? If not, blocked.
  await new Promise(r => setTimeout(r, 6000));
  const orgId = ctx.testorg.id;
  if (!orgId) return { status: 'blocked', details: { reason: 'step 2 did not produce org id' } };
  const { data: runs, error } = await ctx.supabaseService
    .schema('pathfinder')
    .from('agent_runs')
    .select('id, agent_name, organization_id, started_at')
  ...
  if (!runs || runs.length === 0) {
    return {
      status: 'blocked',
      details: { reason: 'no agent_runs rows for new org within 6s — Phase 2E org.created auto-dispatch may not be wired yet' },
    };
  }
  return { status: 'pass', details: { agent_runs_count: runs.length } };
}
...
/** Step 7 — Operator deep-link from Metacron Customers tab is enabled. */
async function step7(ctx: SmokeContext): Promise<Omit<Result, 'step' | 'name' | 'latency_ms'>> {
  // Probe: HEAD the operator-facing Metacron customers list endpoint and look for the test org slug.
  // Without authenticated session this returns 401/403 — that itself confirms the gate is wired.
  const url = `${ctx.endpoints.metacron}/api/customers`;
  const res = await fetch(url, { method: 'HEAD' }).catch(e => ({ ok: false, status: 0, statusText: e?.message ?? 'fetch failed' } as Response));
  if (res.status === 0) return { status: 'fail', error: (res as unknown as { statusText: string }).statusText };
  if (res.status === 401 || res.status === 403 || res.ok) {
    return { status: 'pass', details: { url, http_status: res.status } };
  }
  return { status: 'blocked', details: { url, http_status: res.status, reason: 'metacron customers endpoint unexpected status' } };
}
...
async function step8(ctx: SmokeContext): Promise<Omit<Result, 'step' | 'name' | 'latency_ms'>> {
  ...
  const res = await fetch(url, { redirect: 'follow' }).catch(e => null);
...
async function main(): Promise<void> {
  ...
  const ctx: SmokeContext = {
    ...
    endpoints: {
      pathfinder: process.env.DOD_PATHFINDER_BASE ?? 'https://pathfinder-ashy.vercel.app',
