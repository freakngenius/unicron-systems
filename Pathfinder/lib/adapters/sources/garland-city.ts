// lib/adapters/sources/garland-city.ts
//
// Sprint Z10 adapter — City of Garland Purchasing (DFW suburb, Dallas County).
// Endpoint: https://www.garlandtx.gov/162/Purchasing (CivicPlus CMS)
//
// ENDPOINT DISCOVERY (2026-05-28):
//   Garland runs on CivicPlus. Per the standard CivicPlus pattern, the
//   /162/Purchasing page renders its substantive copy inside `.fr-view`
//   or `.content` regions containing anchor lists for current open
//   solicitations. Garland typically points solicitation links at
//   PDF bid documents and at the BidNet Direct / IonWave / Bonfire
//   portal where vendors register and submit responses.
//
//   ROW SELECTOR: `a[href]` scoped to `.fr-view, .content,
//   #ContentPlaceHolder1` when present; otherwise the whole body after
//   header/nav/footer pruning. Anchors are kept when they look like a
//   solicitation gateway (PDF, BidNet, IonWave, Bonfire, RFP/RFQ/IFB
//   path). Adjacent text is scanned for closing-date phrases so the
//   response_deadline carries through.
//
// FALLBACK STRATEGY:
...
//   skipped — the helper reads HTML body text. Cloudflare / per-URL
//   failures are swallowed.
//
// Geofence: Garland → Dallas County, TX → in-region.
...
const ENDPOINT = 'https://www.garlandtx.gov/162/Purchasing';

const AGENCY = 'City of Garland Purchasing';
const CITY = 'Garland';
...
const PROCUREMENT_HREF =
  /\.(pdf|docx?)($|\?)|bidnetdirect\.com|ionwave\.net|periscopeholdings\.com|bonfirehub\.com|publicpurchase\.com|\/bid|\/rfp|\/rfq|\/ifb|\/itb|\/solicitation|\/purchasing\/.+/i;
...
function extractSolicitationId(text: string, url: string): string | null {
  ...
  const fromUrl = url.match(
    /(?:[?&]bidId=|\/opportunities?\/|\/bid\/|\/rfp\/|\/document\/|\/DocumentCenter\/View\/)([A-Z0-9][A-Z0-9._\-]{2,})/i,
  );
...
export const garlandCityAdapter: SourceAdapter = {
  id: 'garland-city',
  type: 'registered',
  description:
    'City of Garland Purchasing — CivicPlus /162/Purchasing landing-page scrape (anchor + nearby date text, with detail-page phase enrichment).',
