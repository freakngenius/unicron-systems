...
/** Teams Adaptive Card action button feedback (source='teams_card').
 *
 *  Distinct from `recordCommandFeedback` because Teams routes feedback
 *  through Action.Submit clicks, not slash commands. We record source
 *  ='teams_card' per the lead_feedback check constraint added in 0107.
 *  The Teams Activity id (the card the user clicked) is preserved as
 *  source_external_id so duplicate clicks can be deduped at the analytics
 *  layer.
 */
export interface RecordTeamsCardFeedbackArgs {
  customerOrgId: string;
  connectorId: string;
  projectId: string;
  thumb: 'up' | 'down';
  /** Teams Activity id of the card that was clicked. */
  activityId: string | null;
  /** Teams user aad-id (or anonymous principal). */
  userExternalId: string | null;
  reason?: string | null;
}

export async function recordTeamsCardFeedback(args: RecordTeamsCardFeedbackArgs): Promise<void> {
  const sb = supabaseAdmin() as unknown as {
    from: (t: string) => {
  ...
  const r = await sb.from('lead_feedback').insert({
    ...
    reason: args.reason ?? null,
    source: 'teams_card',
    source_external_id: args.activityId,
    user_external_id: args.userExternalId,
    connector_id: args.connectorId,
  });
  if (r.error) {
    if (r.error.code === '23505') return;
    throw new Error(`lead_feedback insert (teams_card) failed: ${r.error.message}`);
  }
}
